...
var addProperty = function addProperty(customElement, name) {
	...
	var descriptor = Object.getOwnPropertyDescriptor(customElement.prototype, name);

	if (descriptor) {
		if (typeof descriptor.set === 'function') {
			var generated = property.set;
			var existing = descriptor.set;

			property.set = function set(to) {
				var _this = this;

				var sync = function sync(parsed) {
					generated.call(_this, parsed);
				};

				existing.call(this, { to: to, sync: sync });
			};
		}

		if (typeof descriptor.get === 'function') {
			var _generated = property.get;
...
var registerAttribute = function registerAttribute() {
	...
	var observer = new MutationObserver(function (mutations) {
		Array.from(mutations, function (mutation) {
			...
			handlers.push(function (mutation) {
				if (mutation.type === 'attributes' && nodeIsSupported(mutation.target)) {
					...
					if (mutation.removedNodes) {
						Array.from(mutation.removedNodes, function (node) {
...
var addAttributesToController = function addAttributesToController(controller) {
	...
	return attributes.map(function (attribute) {
		...
		if ((typeof attribute === 'undefined' ? 'undefined' : _typeof(attribute)) === 'object') {
			...
			var _name = attribute.attribute;

			var _generateAttributeMet2 = generateAttributeMethods(_name, type),
			    _getter = _generateAttributeMet2.getter,
			    _setter = _generateAttributeMet2.setter;

			addProperty(controller, _name, _getter, _setter);
			return _name;
		}
