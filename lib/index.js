...
var SmoothStateController = function SmoothStateController() {
	var SMOOTH_STATE_DOM_PARSER = new DOMParser();

	return function (_BaseController) {
		inherits(_class, _BaseController);
		...
		createClass(_class, [{
			...
			value: function goTo(href) {
				...
				var parseHTML = function parseHTML(html) {
					try {
						var parsed = SMOOTH_STATE_DOM_PARSER.parseFromString(html, 'text/html');

						var title = parsed.title;
						var container = parsed.body.getElementsByTagName('mr-smooth-state');

						var content = container.length === 0 ? parsed.body : container[0];
