...
var registerAttribute = function registerAttribute() {
	...
	return function register(attribute) {
		...
		waitForDOMReady().then(function () {
			...
			var on = options.on || HTMLElement;

			var nodeIsSupported = function nodeIsSupported(node) {
				if (Array.isArray(on)) {
					return on.some(function (supported) {
			...
			Array.from(root.querySelectorAll('[' + attribute + ']'), function (el) {
...
var addAttributeToController = function addAttributeToController(controller, attribute) {
...
function defineCustomElement(tag) {
	...
	// Validate tag
	var validated = tag.split('-').length > 1;
	var type = options.type || 'element';
